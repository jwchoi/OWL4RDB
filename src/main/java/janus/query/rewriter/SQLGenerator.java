...
public abstract class SQLGenerator {
	...
	public String getQueryToGetIndividualsOfClass(URI cls, String columnName) {
		...
		if (Janus.owlMappingMetadata.getClassType(cls).equals(OntEntityTypes.TABLE_CLASS))
			query = getQueryToGetIndividualsOfTableClass(cls, columnName);
		else if (Janus.owlMappingMetadata.getClassType(cls).equals(OntEntityTypes.COLUMN_CLASS))
			query = getQueryToGetIndividualsOfColumnClass(cls, columnName);
		else if (Janus.owlMappingMetadata.getClassType(cls).equals(OntEntityTypes.OWL_THING_CLASS))
	...
	private String getQueryToGetIndividualsOfTableClass(URI cls, String columnName) {
		String table = Janus.owlMappingMetadata.getMappedTableNameToClass(cls);
	...
	private String getQueryToGetIndividualsOfColumnClass(URI cls, String columnName) {
		String table = Janus.owlMappingMetadata.getMappedTableNameToClass(cls);
		String column = Janus.owlMappingMetadata.getMappedColumnNameToClass(cls);
		...
		else if (Janus.localDBSchema.isNotNull(table, column))
	...
	private String getQueryToGetTypesOfFieldIndividual(URI individual, String columnName) {
		...
		URI mappedClass = Janus.owlMappingMetadata.getMappedClass(table, column);
		
		Set<URI> familyClsses = Janus.ontBridge.getAllFamilyClasses(mappedClass);
		
		for (URI cls: familyClsses) {
			String mappedTable = Janus.owlMappingMetadata.getMappedTableNameToClass(cls);
			String mappedColumn = Janus.owlMappingMetadata.getMappedColumnNameToClass(cls);
	...
	private String getQueryToGetTypesOfRecordIndividual(URI individual, String columnName) {
		...
		URI mappedClass = Janus.owlMappingMetadata.getMappedClass(table);
		...
		for (URI cls: familyClsses) {
			...
			for (DBField field: fields) {
				String matchedPKColumn = Janus.localDBSchema.getMatchedPKColumnAmongFamilyTables(field.getTableName(), field.getColumnName(), mappedTable);
	...
	public String getQueryToGetTypesOfIndividual(URI individual, String columnName) {
		...
		if (Janus.owlMappingMetadata.getIndividualType(individual).equals(OntEntityTypes.RECORD_INDIVIDUAL))
			query = getQueryToGetTypesOfRecordIndividual(individual, columnName);
		else if (Janus.owlMappingMetadata.getIndividualType(individual).equals(OntEntityTypes.FIELD_INDIVIDUAL))
	...
	public String getQueryToGetAllPropertyAssertionsOfSourceIndividual(URI srcIndividual, String pColumnName, String tColumnName) {
		...
		if (Janus.owlMappingMetadata.getIndividualType(srcIndividual).equals(OntEntityTypes.RECORD_INDIVIDUAL)) {
		...
		} else if (Janus.owlMappingMetadata.getIndividualType(srcIndividual).equals(OntEntityTypes.FIELD_INDIVIDUAL))
	...
	private String getQueryToGetOPAssertionsOfFieldTargetIndividual(URI individual, String pColumnName, String sColumnName) {
		...
		for (URI cls: familyClsses) {
			String familyTable = Janus.owlMappingMetadata.getMappedTableNameToClass(cls);
			String familyColumn = Janus.owlMappingMetadata.getMappedColumnNameToClass(cls);
	...
	private List<String> getQueriesToGetOPAssertionsOfRecordSourceIndividual(URI individual, String pColumnName, String tColumnName) {
		...
		for (URI cls: familyClsses) {
			...
			Set<String> keyColumns = Janus.localDBSchema.getKeyColumns(mappedTable);

			for (String keyColumn: keyColumns) {
				URI mappedOP = Janus.owlMappingMetadata.getMappedObjectProperty(mappedTable, keyColumn);
	...
	private List<String> getQueriesToGetDPAssertionsOfRecordSourceIndividual(URI individual, String pColumnName, String tColumnName) {
		...
		for (URI cls: familyClsses) {
			...
			Set<String> nonKeyColumns = Janus.localDBSchema.getNonKeyColumns(mappedTable);

			for (String nonKeyColumn: nonKeyColumns) {
				URI mappedDP = Janus.owlMappingMetadata.getMappedDataProperty(mappedTable, nonKeyColumn);
	...
	private List<String> getQueriesToGetDPAssertionsOfFieldSourceIndividual(URI individual, String pColumnName, String tColumnName) {
		...
		for (URI cls: familyClsses) {
			...
			URI mappedDP = Janus.owlMappingMetadata.getMappedDataProperty(mappedTable, mappedColumn);
	...
	public String getQueryToGetDPAsserionsOfDP(URI dp, String sColumnName, String tColumnName) {
		...
		URI domainCls = Janus.owlMappingMetadata.getDomainClassOfProperty(dp);
		
		OntEntityTypes type = Janus.owlMappingMetadata.getClassType(domainCls);
	...
	public String getQueryToGetDPAssertionsOfTargetLiteral(String literal, String pColumnName, String sColumnName) {
		String lexicalValue = Janus.owlMappingMetadata.getLexicalValueOfTypedLiteral(literal);
		String datatypeOfTypedLiteral = Janus.owlMappingMetadata.getDatatypeOfTypedLiteral(literal);
		
		Set<DBColumn> dbColumns = Janus.owlMappingMetadata.getMappedDBColumnsToDatatypeOfTypedLiteral(datatypeOfTypedLiteral);
		
		List<String> queries = new Vector<String>();
		
		for(DBColumn dbColumn: dbColumns) {
			
			if (Janus.localDBSchema.isKey(dbColumn.getTableName(), dbColumn.getColumnName()))
	...
	public String getQueryToGetAllClsAssertions(String columnName1, String columnName2) {
		...
		for (String table: tables) {
			...
			Set<String> keys = Janus.localDBSchema.getKeyColumns(table);
			
			for (String key: keys) {
				if (Janus.localDBSchema.isRootColumn(table, key))
					queries.add(getQueryToGetAllClsAssertionsOfColumnClass(table, key, owlThing, columnName1, columnName2));
				
				mappedClass = Janus.owlMappingMetadata.getMappedClass(table, key);
	...
	public String getQueryToGetObjectPropertiesOfOPAssertion(URI aSourceIndividual, URI aTargetIndividual, String columnName) {
		...
		DBField targetField = Janus.owlMappingMetadata.getMappedDBFieldToFieldIndividual(aTargetIndividual);
		
		Set<DBColumn> familyColumnsOfTarget = Janus.localDBSchema.getFamilyColumns(targetField.getTableName(), targetField.getColumnName());
		
		Set<DBColumn> columnsInFamilyTablesOfSrc = Janus.localDBSchema.getColumsIncludedInTables(familyTablesOfSrc, familyColumnsOfTarget);
		...
		for (DBColumn aColumn: columnsInFamilyTablesOfSrc) {
			...
			for (DBField srcField: srcFields) {
				String matchedColumn = Janus.localDBSchema.getMatchedPKColumnAmongFamilyTables(srcTable, srcField.getColumnName(), table);
	...
	public String getQueryToGetDataPropertiesOfDPAssertion(URI aSourceIndividual, String aTargetLiteral, String columnName) {
		OntEntityTypes individualType = Janus.owlMappingMetadata.getIndividualType(aSourceIndividual);
	...
	private String getQueryToGetDataPropertiesOfDPAssertionWithFieldSrcIndividual(URI aSourceIndividual, String aTargetLiteral, String columnName) {
		...
		if (!mappedSQLTypes.contains(Janus.localDBSchema.getJDBCDataType(srcTable, srcColumn))
				|| !srcValue.equals(valueOfTargetLiteral))
			return getUnionQuery(queries, columnName);
		
		Set<DBColumn> familyColumnsOfSrc = Janus.localDBSchema.getFamilyColumns(srcField.getTableName(), srcField.getColumnName());
	...
	private String getQueryToGetDataPropertiesOfDPAssertionWithRecordSrcIndividual(URI aSourceIndividual, String aTargetLiteral, String columnName) {
		...
		for (String table: familyTablesOfSrc) {
			Set<String> columns = Janus.localDBSchema.getNonKeyColumns(table);
